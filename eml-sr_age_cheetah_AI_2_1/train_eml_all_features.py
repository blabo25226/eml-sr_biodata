...
def main():
    ...
    # Use all features (exclude target columns)
    feature_cols = [col for col in df.columns if col not in ['Age_trans', 'Age']]
    ...
    # Use new model-first AI engine, increased beam width for handling 51 variables
    searcher = eml_sr.Searcher(max_complexity=10, beam_width=1000)
    # Fit the searcher
    ...
    # --- Save Model info ---
    with open('eml_model_results_all_features.txt', 'w') as f:
    ...
    plt.tight_layout()
    plt.savefig('model_comparison_all_features.png')
    plt.close()
    
    print("Training complete. Results saved to model_comparison_all_features.png and eml_model_results_all_features.txt")
